...
- Explorar RMarkdown
- Introduccion a las funciones básicas incluidas en R
...
cada vez herramientas de programación ed código abiermo mas faciles de usar y 
potentes. El progreso actual en realcion a las herramients de programación permiten que 
aprender a escribir códigos computacionales sea cada vez más facil. Incorporando la 
programción a el procesos de análisis no solo hace que la ciencia sea más eficientes, 
si no que además sea computacionalmente reproducible. En este curso, vamos a usar 
el lenguaje de programa R y para esto usaresmos el Ambiente de Programación Integrada
(integrated development environment o IDE) RStudio. R es un lenguaje ideal para aprenedr
programación orientada al los datos, porque es ampliamente utilizada por la comunidad
científica, es amigable con el usuario (user-friendly) y lo mas importante es de código
abierto (open source). 
...
(peso_kg <- 55)  # but putting parenthesis around the call prints the value of `weight_kg`
...
### Los mensajes de arror son tus amigo
