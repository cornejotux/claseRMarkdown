...
sabe como funciona? Afortunadamente RStudio entrega una forma myu fácil para acceder
...
Esto va a abrir el panel de ayuda en la seccion inferior derecha de RStudio.

Las páginas de ayuda están dividads en secciones, la documentación está en inglés:

 - Description: Se entrega una descripción extendidad de lo que hace la función.
...
funciones que usted estara que existan. Aquí les dejo algunas ideas: `?getwd()`,
`?plot()`,  `min()`, `max()`, `?log()`.
...
del código que escribamos. Esta función hace exacatamente lo que dice (en inglés!),
lee un archivo _csv_ a R.

Ya que esta será nuestra primera vez usando esta función, promero leeremos la página de 
ayuda de `read.csv`. La ayuda tiene mucha información y acepta muchos argumentos, sin 
embargo el primedo de ellos es el mas improtante - tenemos que decirle que archivo
es el que va a leer. 

#### Descarge un archivo desde el Arctic Data Center

Nevege al set de datos de Craig Tweedie que está publicado en el Arctic Data Center. 
...
Mueva este archivo desde su carpeta de Descargas a un directorio de facil acceso. 
Yo recomendo crear un directorio llamado `datos` en el directorio que creo anteriormente
...
leer. para esto usarl el argumento `file` (archivo) que se puede ver en la sección 
usage (uso) de la página de ayuda. En RMarkdown, se puede usar path absolutos (que comienzan
con su directorio `~/`) o path relativaso, que son **relativos a la ubicaciñon del archivo
RMarkdown**. RStudio y RMarkdown tiene la capacidad de autocompletar cuano se usar paths
relativos, por lo que usareoms ese sistema. Asumiento que ya movió el archivo a el directorio
dentro de la carpeta `archivos_clase_R` que se llama `datos`, su función `read.csv`
puede ser llamada de la siguiente forma:

```{r, eval = F}
bg_chem <- read.csv("datos/BGchem2008data.csv")
```
...
Note que en las páginas de ayuda hay una gran catidad de argumentos que no fue necesario 
...
`nombre = valor` para ese argumento cuando llame a la función, la funsión asumirá el valor
...
es el primer argumento. Si queremos agregar algun otro argumento, por ejemplo 
...
uso esto solo para los primeros 2 argumento, para argumento más alla siempre uso el par
`nombre = valor`.

Muchos usuraio de R (incluyendome) sobreescriben el argumento por defecto de `stringsAsFactors`
