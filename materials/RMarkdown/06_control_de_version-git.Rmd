...
Hagamos la distinción entre **git** y **GitHub**.

- __git__: Software para el control de versiones que monitorea los archivos de un 
directorio (un repositorio)
    - git crea el historial de versiones de un repositorio
- __GitHub__: Sitio web que permite a usuarios almacenar sus repositorios git y compartirlos
con otros
...
La captura de pantalla en la figura \@ref(fig:webGitHub) muestra una copia de un repositorio 
...
Monitorear estos cambios, como se relacionan a cada las versiones de un software en 
...
versiones de códigos científicos, figuras y manuscritos para lograr tener
...
se mueven a través del repositorio.  Cualquier repositorio de git puede ser
...
versión actual de todos los archivos del repositorio. Estos archivos de trabajo
...
GitHub creó por usted, por ejemplo los archivos README.md, LICENSE y .gitignore.
...
Para hacer cambios menores a archivos de texto, se puede utilizar la interfase web 
de GitHub.  Por ejemplo, navegue al archivo `README.md` en el listado de archivos 
y habilite la edición haciendo clic en el icono del _lápiz_. Este es una archivo
normal con el formado Markdown, ahora se puede editar, agregando o removiendo texto.
Cuando haya terminado, incluya un mensaje de commit y luego haga clic en el botón
`Commit changes`.  
...
repositorio GitHub, puede ver la lista de los cambios confirmados (commits) ahí, así como 
...
Expliquemos algunas cosas sobre esta ventana. Representa una vista de el repositorio que
...
En la siguiente sección vamos a usar el URL de GitHub para el repositorio que creo y 
...
(RStudio project folder).  En esta sección vamos a clonar el repositorio que se creo
en GitHub y lo vamos a dejar un repositorio local de un proyecto de RStudio.  
...
1. Cree y confirma (commit) un archivo Rmd
...
que el que se tiene en GitHub, de esta forma se mantiene un grado de correspondencia.
...
se abrirá con todos los archivos copiados localmente desde el repositorio remoto. 
Dependiendo de como esté configurada la versión de RStudio, la posición 
y tamaño de los paneles puede cambiar, pero generalmente todos van a estar presentes,
incluyendo los paneles *Git* y el listado de *Files* que fueron creados en el
repositorio remoto.
...
modificaciones. En este caso, el archivo `.gitignore` se muestra con una
...
clic en el botón *History* (historial), con el que le van a mostrar el 
...
archivo original. La líneas nuevas se destacan en color verde y en rojo 
...
`sasap-test.Rproj`, aun se identificación que no están confirmados 
...
mensaje de confirmación (commit message; fig \@ref(fig:rstudioCommit2)).
...
botón *Push*.  Esto hará que se le pregunte por su usuario y password de GitHub y
...
los commits, incluyendo los 2 que fueron hechos en GitHub y los 2 que se hicieron en
...
Es claro que una buena documentación es crítica para hacer del historial de versiones
significativo y útil. Es tentador saltarse la escritura del mensaje asociado al 
commit o escribir algo por defecto como 'Actualización!'. Sin embargo es mucho
mejor escribir mensajes que serán útiles para ti en el futuro, tal que te 
entregue información que te permita sabe no solo que fue lo que hiciste, si no que
además se pueda entender el por qué. Ademas, los mensajes que se usan en commits
son en general más fáciles de entender si usan una convención de verbos activos. 
Por ejemplo, se puede ver que los mensajes de commit en las capturas de pantallas 
comienzan siempre con un verbo en pasado (en inglés!) y que explican que fue 
lo que se cambió.

Si bien mucho de los cambios que aquí se hicieron son simples y se explican por 
si mismos, para cambios más complejos, es mejor entregar un mensaje completo y 
auto-contenido. La convención, sin embargo, es tratar de usar mensajes cortos, 
con una sentencia breve, seguido de una explicación más detallada y racional 
para el cambio. Esto permite que el nivel de detallas seas leíble en el registro
de las versiones (version log). No puedo contar el número de veces que he visto
los registro de commits de hace 2, 3 o 10 años y agradecido el nivel de diligencia
de los colaboradores que se tomaron el tiempo de describir el trabajo que 
realizaron.

## Flujos de trabajo colaborativos y libres de conflictos (relacionados con Git)

Hasta ahora nos hemos enfocado a usar Git y GitHub sólo para nosotros, esto es muy 
útil. Pero aún más poderoso es compartir un repositorio con GitHub con otros
investigadores, tal que se puede trabajar en un código, análisis y modelos en forma
colaborativa. Cuando se trabaja de esta manera con otros colaboradores, es muy
importante poner atención al estado del repositorio remoto para evitar potenciales
conflictos al combinar el trabajo. Un *merge conflict* ocurre cuando dos colaboradores
hace 2 commits en forma separada que han cambiado las misma líneas de código de un 
archivo. Cuando esto ocurre, git no puede combinar los cambios en forma automática 
y arroja un error preguntando como resolver el conflicto. Esto no es preocupante, 
no le tenga miedo a los _merge conflicts_ ya que son muy fáciles resolver y aquí
hay 
[guías](https://help.github.com/articles/resolving-a-merge-conflict-using-the-command-line/) [geniales](https://stackoverflow.com/questions/161813/how-to-resolve-merge-conflicts-in-git).
de como resolverlos (por ahora sólo en inglés).

Dicho esto, es siempre mucho mejor evadir este tipo de conflictos y se pueden minimizar
siguientes las siguientes sugerencias:
...
- Coordínese con con sus colaboradores en quien va a trabajar en que archivos
...
Hay mucho que acá no se ha visto en este tutorial. Existen tutoriales muy 
completos que cubre tópicos mas avanzados, hasta ahora tengo link a versiones en 
inglés que espero poder reemplazar con otras en español más adelante. Alguno
de estos temas son:
