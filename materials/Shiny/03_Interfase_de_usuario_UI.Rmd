...
- Incluir imagenes en la interfase de usuario.
...
```
La Shiny app creada con esta interfase de usuario resultante se puede ver la Figura 3.1.


![Figura 3.1: Interfase de Usuario (UI) limpia y sin contenido.](imagenes/3.1_Shiny_Clear.png)
...
La barra lateral aparecerá por defecto en el lago izquierdo de la aplicación. Esta se puede mover al lado derecho agregando a la función `sidebarLayout` el argumento `position = "right"`. Este código se puede ver a continuación y el resultado en la Figura 3.2.
...
![Figura 3.2: Interfase de Usuario (UI) limpia y sin contenido, con la barra lateral a la derecha, usando el argumento `position = "right"`.](imagenes/3.2_Shiny_ClearRight.png)
...
La Sihny app resultante debería ser similar a lo que se predenta en la Fig. 3.3.

![Figura 3.3: Interfase de Usuario (UI) utilizando los 6 niveles de títulos (h1 a h6).](imagenes/3.3_Shiny_Titles.png)
...
La Shiny app resultate con esta ui se puede ver en la fig. 3.4

![Figura 3.4: Interfase de Usuario (UI) donde se muestran ejemplos de como formatear texto.](imagenes/3.4_Shiny_FormatText.png)

### Imágenes

Incluir imágenes en el Shiny app puede mejorar significativamente la apariencia de la aplicación y ayudar a los usuarios 
a entender mejor el contenido. Shiny permite agregar imágenes utilizando la función `img`.

Para incluir una imagen use la función `img()`, se debe incluir en el argumento src el path y el nombre del archivo utilizando
comillas (ej. img(src = "Path/a/mi/Imagen.png")). Esto tambien se puede hacer utilizando un _url_ si la imagen esta dispoble 
en una página web. A esta función tambien se le pueden pasar identificadores HTML como alto (_height_) y ancho (_width_), con 
...
con el navegadore del usuario, lo que lo hace el lugar perfecto para poner imágenes, hojas de estilo y otras cosas que el 
navegador puede necesitar para contruir los componentes web de la Shiny app.

Un código de ejemplo con una imagen local y otra remota se muestra a continuación. Baje el archivo rstudio.png desde [[aquí]{https://shiny.rstudio.com/tutorial/written-tutorial/lesson2/www/rstudio.png}], déjelo en el directorio _www_ y úselo. La imagen remota no necesita ningún proceso o cambio para que la pueda utilizar. 

> Por favor note que entre las 2 funciones img() existe una coma, esta es necesaria, de otra forma R lo interpreta coom solo una función y generará un error.
...
```
![Figura 3.5: Interfase de Usuario (UI) con una imgaen local en directorio _www_ y una remota en un servidor web.](imagenes/3.5_Shiny_Img.png)

### Otros Identificadores (tags) de funciones

En este capítulo cubrimos los identificadores de funciones Shiny más comunes y usados, pero existen mucho otros que pueden ser utilizados. Puede aprender sobre estos otros identificadores de funcioes en [Customize your UI with HTML](https://shiny.rstudio.com/articles/html-tags.html){target=_new} y en [Shiny HTML Tags Glosay](https://shiny.rstudio.com/articles/tag-glossary.html){target=_new}.

## Actividad

Cree una Shiny app utilizando lo que ha aprendido sobre el diseño de los paneles, HTML y funciones de imagen para crear una interfase de uruario atractiva. Vea cuando bien entiende estas funciones recreando la Shiny app que se presenta en la figura 3.6. Utilice los ejemplos con los que hemos trabajado durante el capítulo.

![Figura 3.6: Imagen de la Shiny app objetivo para la actividad de aprendizaje-](imagenes/3.6_Actividad.png)
...
- Defiinr atributos identificadores de HTML en los argumento de cada función de identificación.
- Agregar un elemento a tu página web pasándoselo a las funciones `titlePanel`, `sidebarPanel` o `mainPanel`.
- Agregar múltiples elemento a cada panel al separarlos con una coma.
- Usar la función `img()` para agregar imágenes existentes en la carpeta _www_ o en un servidor web a su aplicación Shiny app.

Ahora que ya puede agrear contenido en su ui, ahora tenemos que ver como trabajar con contenido mas complejo, como complementos (_widgets_). Widgets son los elemento interactivos que el usuario manipula para controlar la aplicación.
