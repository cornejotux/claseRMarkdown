...
Para instalar un nuevo paquete de R se pueden descargar desde CRAN utilizando la línea de comandos (como se hace en R)
...
en Install, esto abre una ventana donde se puede escribir las librerías que necesitamos. En el ejemplo en la figura
...
Una de los aspectos más útiles que a mi parecer tiene RStudio son los _Proyectos_ (Project en inglés). Esto es un conjuto de
...
creación de un nuevo proyecto, donde trabajará con sus archivos de código. Estos códigos son los que luego se usan para 
cargar datos, crear gráficos, producir tablas, exportar datos, etc.

Uno de los aspectos más útiles que tiene la creación de un proyecto es que deja el directorio de trabajo (equivalente a
a la funcion _setwd_) relativo a donde fue creado el proyecto. De esta forma, se puede ahorrar escribir esos path largos
antes de leer/escribir cada archivo. Además, los archivos de código que se generen en el proyecto, estarán siempre 
disponibles para consultarlos en cualquier momento que el usuario así lo desee. Para crear un proyecto, vamos a la pestaña
superior derecha de Rstudio y damos clic en crear un proyecto (fig. \@ref(fig:rstudioNewProject)).
...
(New File) -> R Script (RScrit). 

Nose que en IDE RStudio se puede trabajar con muchos otros lenguajes y convenciones (Fig. \@ref(fig:rstudioNewFile)).

```{r rstudioNewFile, echo=FALSE, out.width = '80%', fig.align = 'center', fig.cap = 'Creación de un archivo nuevo en RStudio.'}
